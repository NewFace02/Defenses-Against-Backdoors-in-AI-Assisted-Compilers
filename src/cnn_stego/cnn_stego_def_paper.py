...
def quantize_blocks(w, bits=8, block_size=32):
...
def apply_defense_to_param(
    ...
    q, scale, orig_shape, pad_len, writable, qmax = quantize_blocks(backup, bits=q_bits, block_size=block_size)
...
def estimate_pai_for_param(model, param_name, loader, device, q_bits, block_size, probe_positions, base_acc, base_logits, max_batches, seed):
    p = get_param(model, param_name)
    backup = p.detach().clone()
