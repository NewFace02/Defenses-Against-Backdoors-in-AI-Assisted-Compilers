...
# -*- coding: utf-8 -*-
"""
Final Llama-3.1-8B INSPECT-Opt defense script.
...
def comp_indices_for_param(p: torch.nn.Parameter, hvec: torch.Tensor, q: torch.Tensor, q_bits: int, top_hidden_dims: int, use_quant_safety: bool = True):
    if bool(use_quant_safety):
        stable = BlockQuantizer.stable_payload_mask_from_q(q, n=1, q_bits=q_bits)
        valid_mask = stable.reshape(-1)
    else:
        valid_mask = torch.ones(q.reshape(-1).numel(), dtype=torch.bool, device=q.device)
...
def stego_stream_indices_for_layer(layer_params, payload_bits_len: int, q_bits: int, n: int, use_quant_safety: bool = True):
    ...
    for local_name, p in layer_params:
        ...
        if bool(use_quant_safety):
            ...
            real_n = int(q.numel() - int(_pad))
            valid = torch.arange(real_n, device=p.device, dtype=torch.long)
            stable = None
    ...
    for li in union_layers:
        ...
        if li in stego_layer_set:
            stego_idx_by_local = stego_stream_indices_for_layer(
                ...
                use_quant_safety=(not bool(getattr(args, "disable_quant_safety", False)))
            )
        ...
        for local_name, p in layer_weight_parameters(layer):
            ...
            if li in comp_layer_set:
                comp_idx = comp_indices_for_param(p, hvec, q, args.q_bits, args.comp_top_hidden_dims, use_quant_safety=(not bool(getattr(args, "disable_quant_safety", False))))
...
def inspect_optimize(model, tokenizer, runner: ChatGLMSASER, states: List[TensorOptState], args):
    ...
    # Random ablation: directly assign random continuous u values, then reuse
    # the original apply_discrete_purification() to compute Δ and statistics.
    if str(getattr(args, "ablation_mode", "none")) in {"random_same_k", "random_large_k"}:
        flat = []
        ...
        base_k = int(getattr(args, "random_selected_count", 0) or 0)
        if base_k <= 0:
            base_k = int(len(flat))

        if str(args.ablation_mode) == "random_large_k":
            requested = int(base_k * int(getattr(args, "random_large_mult", 10)))
        else:
            requested = int(base_k)

        actual = min(int(requested), int(len(flat)))
        ...
        with torch.no_grad():
            ...
            if actual > 0:
                perm = torch.randperm(len(flat), generator=g)[:actual].tolist()
                for idx in perm:
                    si, local_j = flat[int(idx)]
                    sign = 1.0 if int(torch.randint(0, 2, (1,), generator=g).item()) == 1 else -1.0
                    # Make sure |u| > tau, so original discretization selects it.
                    states[si].u.view(-1)[int(local_j)] = float(sign)

        print(f"[Random Ablation] mode={args.ablation_mode}, requested={requested}, actual={actual}, available={len(flat)}")
        return pd.DataFrame([{
            "step": 0,
            "loss": None,
            "lattack": None,
            ...
            "ltri": None,
            "selected": int(actual),
            "random_requested": int(requested),
            "random_selected": int(actual),
            "random_available": int(len(flat)),
...
def main():
    ...
    parser.add_argument("--ablation-mode", choices=["none", "random_same_k", "random_large_k"], default="none")
    parser.add_argument("--random-selected-count", type=int, default=0)
    parser.add_argument("--random-large-mult", type=int, default=10)
    parser.add_argument("--disable-quant-safety", action="store_true", help="Ablation: disable stable/writable low-bit candidate mask; keep q clamp for numeric safety.")
    ...
    result = {
        ...
        "inspect_opt": {
            ...
            "lr": args.lr,
            "ablation_mode": str(getattr(args, "ablation_mode", "none")),
            "random_selected_count": int(getattr(args, "random_selected_count", 0)),
            "random_large_mult": int(getattr(args, "random_large_mult", 10)),
            "disable_quant_safety": bool(getattr(args, "disable_quant_safety", False)),
            **stat,
